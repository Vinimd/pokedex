...
export default function ChangeType(item) {
  const isByType =
    item.includes("tipo") ||
    item.includes("próximo") ||
    item.includes("anterior");
  const containerTypeSearch = document.querySelector(".search-type");

  function TypeRequest(speech, type) {
    ...
    typeUrl
      ...
      .then((json) => {
        ...
        if (index < json.pokemon.length - 1 && speech.includes("próximo")) {
          index++;
        } else if (index > 0 && speech.includes("anterior")) {
          index--;
        } else if (
          index == json.pokemon.length - 1 &&
          speech.includes("próximo")
        ) {
          index = 0;
        } else if (index == 0 && speech.includes("anterior")) {
          index = json.pokemon.length - 1;
      ...
      .catch(() => {
        ...
        divError.innerHTML =
          "<span><strong>Tipo Não Encontrado</strong></span>";
  ...
  isByType ? ChangePokemonType() : SearchByName();
}
