...
export default function InitSearchBytype() {
    ...
    typeUrl
      ...
      .then((json) => {
        ...
        if (
          ...
          event.target.dataset.serachType == "next"
        ) {
          index++;
          ...
          index === 0 &&
          event.target.dataset.serachType === "previous"
        ) {
          index = json.pokemon.length - 1;
        }
  ...
  function ChangeType(event) {
    ...
    if (numberType < 18 && event.target.dataset.serachType === "next-type") {
      ...
      numberType > 1 &&
      event.target.dataset.serachType === "previous-type"
    ) {
      numberType--;
    } else if (
      numberType === 18 &&
      event.target.dataset.serachType === "next-type"
    ) {
      numberType = 1;
    } else if (
      (numberType === 1 || numberType === 0) &&
      event.target.dataset.serachType === "previous-type"
    ) {
      numberType = 18;
    }
